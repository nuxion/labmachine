...
from labmachine.clusters.types import (BlockInstance, BlockStorage,
                                       MachineInstance, MachineRequest,
                                       StorageRequest)
...
class GCEProvider(ProviderSpec):
    ...
    def _create_boot_disk(
            self, machine_name: str, *, volumes: List[BlockStorage]
    ...
    def create_machine(self, node: MachineRequest) -> MachineInstance:
        ...
        boot = self._create_boot_disk(node.name, volumes=node.volumes)
        boot_name = None
        if boot:
            boot_name = boot.name
    ...
    def attach_volume(self, node: MachineInstance, disk: BlockStorage) -> bool:
        _node = [n for n in self.driver.list_nodes() if n.name ==
                 node.machine_name][0]
